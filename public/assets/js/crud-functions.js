...
function navegarA(seccion) {
    ...
    if (seccionActiva) {
        ...
        // Cargar datos automáticamente según la sección
        setTimeout(() => {
            if (!window.crudFunctions) return;

            switch(seccion) {
                case 'dashboard':  window.crudFunctions.cargarResumenes(); break;
                case 'usuarios':   window.crudFunctions.cargarUsuarios(); break;
                case 'categorias': window.crudFunctions.cargarCategorias(); break;
                case 'productos':  window.crudFunctions.cargarProductos(); break;
                case 'ordenes':    window.crudFunctions.cargarOrdenes(); break;
            }
        }, 500);
    }
    ...
    const menuActivo = document.querySelector(`.dashboard-nav a[href="#${seccion}"]`);
    if (menuActivo) {
...
function inicializarNavegacion() {
    ...
    // Agregar eventos click a TODOS los enlaces del sidebar
    document.querySelectorAll('.dashboard-nav a').forEach(link => {
...
document.addEventListener('DOMContentLoaded', function() {
    setTimeout(inicializarNavegacion, 300);
});